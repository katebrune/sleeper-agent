...
export class GetSleeperRostersTool extends Tool<Args> {
  constructor(private readonly sleeperService: SleeperService) {
    super({
      ...
      description:
        'Get all sleeper rosters for a specific league. Each roster is associated with a user via the owner_id, which maps to a users user_id',
  ...
  execute: ToolFn<Args, string> = async ({ toolArgs }): Promise<string> => {
    try {
      ...
      return `Something went wrong when trying to get roster information about ${toolArgs.league_id}: ${error}`;
    }
  };
}
