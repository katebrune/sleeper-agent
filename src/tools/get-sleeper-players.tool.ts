...
const getSleeperPlayersArgs = z.object({
    player_ids: z.array(z.string().describe('A sleeper player id. E.g. "7549". These are available via rosters, in the players, starters, and reserve properties')).min(1)
})
...
export class GetSleeperPlayersTool extends Tool<Args> {
    ...
    execute: ToolFn<Args, string> = async ({ toolArgs }): Promise<string> => {
        try {
            ...
            return `Something went wrong when trying to get information about ${toolArgs.player_ids}`
        }
    }
}