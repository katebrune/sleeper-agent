...
const getSleeperLeaguesArgs = z.object({
  user_id: z
    .string()
    .describe('The user id of the sleeper user to retrieve leagues for'),
});
...
export class GetSleeperLeaguesTool extends Tool<Args> {
  ...
  execute: ToolFn<Args, string> = async ({ toolArgs }): Promise<string> => {
    try {
      ...
      return `Something went wrong when trying to get league information for ${toolArgs.user_id}: ${error}`;
    }
  };
}
