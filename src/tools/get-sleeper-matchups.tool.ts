...
const getSleeperMatchupsArgs = z.object({
  league_id: z
    .string()
    .describe('The id of the sleeper league to retreive all rosters for'),
  week: z
    .string()
    .describe(
      'The week of the nfl season to get the sleeper fantasy matchups for',
    ),
});
...
export class GetSleeperMatchupsTool extends Tool<Args> {
  constructor(private readonly sleeperService: SleeperService) {
    super({
      ...
      description:
        'Get all sleeper fantasy matchups for a specific league and specific week.',
  ...
  execute: ToolFn<Args, string> = async ({ toolArgs }): Promise<string> => {
    try {
      const data = await this.sleeperService.findMatchups({
        league_id: toolArgs.league_id,
        week: toolArgs.week,
      });
      return JSON.stringify(data);
    } catch (error) {
      return `Something went wrong when trying to get matchup information about ${toolArgs.league_id} for week ${toolArgs.week}: ${error}`;
    }
  };
}
