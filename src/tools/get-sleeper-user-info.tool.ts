...
export class GetSleeperUserInfoTool extends Tool<Args> {
    ...
    execute: ToolFn<Args, string> = async ({ toolArgs }): Promise<string> => {
        try {
            const data = await this.sleeperService.findUser({ username: toolArgs.username })
            return JSON.stringify(data)
        } catch (error) {
            return `Something went wrong when trying to get information about ${toolArgs.username}`
        }
    }
}